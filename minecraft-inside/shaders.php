...
$stmt = $pdo->query("
    ...
    WHERE f.status = 'approved' AND c.type = 'shaders'
...
<div class="container">
    <h1>Шейдеры для Minecraft</h1>
    <p>Улучшите графику игры до невероятного уровня с помощью шейдеров</p>
    
    <div class="files-grid">
        <?php if(empty($files)): ?>
            <div class="no-results">
                <h3>Шейдеров пока нет</h3>
                <p>Будьте первым, кто <a href="upload.php">добавит шейдеры</a></p>
            </div>
        <?php else: ?>
